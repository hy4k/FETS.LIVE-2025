...
const NewWeeklyRosterView: React.FC<WeeklyRosterViewProps> = ({
  ...
  const weeks = useMemo(() => {
    ...
    while (currentWeekStart <= lastDay) {
      ...
      const hasCurrentMonthDays = weekDates.some(date => date.getMonth() === month)
      
      if (hasCurrentMonthDays) {
        weeks.push({
          id: `week-${weekNumber}`,
          label: `Week ${weekNumber}: ${currentWeekStart.toLocaleDateString('en-US', { month: 'short', day: 'numeric' })} – ${weekEnd.toLocaleDateString('en-US', { month: 'short', day: 'numeric' })}`,
